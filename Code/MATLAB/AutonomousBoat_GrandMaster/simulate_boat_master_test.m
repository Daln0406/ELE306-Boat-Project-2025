...
%% ---------- PARAMETRE ----------
dt               = 0.05;       % tidssteg [s] sett 0.005 for "smoothere" visualisering i Gazebo 
snake_speed      = 0.5;        % fremdrift [m/s] (konstant i denne modellen)
plot_update_sec  = 0.05;       % plott hvert 0.05 s
max_time         = 1200;       % maks simuleringstid [s]
USE_EKF_IN_CONTROL = false;    % true=PID bruker theta_est, false=bruker sann theta

% PID
Kp = 5.5;                      % forsterkning 
Ki = 0.2;                      % integrasjonstid
Kd = 0.1;                      % derrivasjonstid

% LOS
L_look = 0.8;                  % LOS-avstand [m]
K_ct   = 2.5;                  % kryss-spor forsterkning
...
snake_path_orig = snake_path;
s_path_orig     = s_path;
s_prog_orig     = 0;          % vi oppdaterer denne når vi avbryter
...
%% ---------- IMU + EKF ----------
Q = diag([0.01 0.01 0.001]);  % Prosess-støy, usikkerhet i modellens dynamikk [x,y,theta]

% Måle-støy: egne variabler for heading-støy
R_mag_heading  = 0.02;        % varians for magnetometer-heading

P = eye(3)*0.1;               % Kovariansmatrise start usikkerheten i estimatet
x_hat = [x; y; theta];        % Startverdien for tilstandsestimatet i Kalman-filteret (pos og heading) 
...
% Plast-markør (start som NaN → usynlig)
...
% --- Strømpil med korrekt retning og skalert lengde ---
x_arrow = 5.0;
...
% Tegn selve pila
hCurrent = quiver(x_arrow, y_arrow, vx_plot, vy_plot, 0, ...
    'Color',[0 0.6 1], 'LineWidth',2, 'MaxHeadSize',2.5, 'ShowArrowHead','on');

% Tekst ved pilen 
text(x_arrow + vx_plot*0.3, y_arrow + vy_plot*0.3, ...
    sprintf('Strøm: %.2f m/s', norm_v), ...
    'Color',[0 0.6 1], 'FontSize',9, 'FontWeight','bold');

%legend([hPath, hBoat, hTrail], {'Snake path','Boat','Trail'}, 'Location','best');
...
while t < max_time
   ...
   % ---------- STYRING + KINEMATIKK ----------
    if mode == "snake" || mode == "goto_plastic" || mode == "return_to_snake"
        ...
        % Hvis du vil kan du la båten drive litt med strømmen:
        % x = x + v_current(1)*dt;
        % y = y + v_current(2)*dt;
        % (ellers bare la x,y være som de er)
    end
    ...
    switch mode
        case "snake"
            ...
            if t > 5 && isempty(plastic_world)
                % 1) Kameramåling i C-frame (fake: plast 1 m rett foran kamera)
                pC = [1.0; 0.0; 0.0];
                ...
                % Husk hvor vi var på snake-banen da vi avbrøt
                s_prog_orig = s_prog;
                boatPose_at_snake_interrupt = boatPose;   % [x y theta]

                % 3) Planlegg Lattice-bane fra nåværende posisjon til plast
        ...
        case "pick"
            % 1) Ny (ev. mer nøyaktig) kameramåling – fortsatt fake
            pC_close = [1.0; 0.0; 0.0];

            % 2) Kamera -> arm-base
            pA = camToArm(pC_close, boatPose, T_BC, T_BA);

            % Litt "safety": begrens rekkevidde i horisontalplanet
            r = norm(pA(1:2));
            r_max = 0.8;               % maks rekkevidde vi prøver (tilpass etter armen)
            if r > r_max
                pA(1:2) = pA(1:2) * (r_max/r);
            end

            % 3) IK på vår 5DOF arm
            robot = boat_arm_model();

            targetPos = pA;
            targetPos(3) = targetPos(3) + 0.05;   % 5 cm over plasten

            R_des = eye(3);   % kan justeres senere
            T_des = [R_des, targetPos;
                     0 0 0 1];

            mask = [1 1 1 0 0 1];       % pos + yaw
            q0   = zeros(1, robot.n);   % startgiss

            % IK – kan feile, så vi sjekker resultatet
            q_sol = robot.ikine(T_des, q0, 'mask', mask);

            disp('q_sol (plukk-konfigurasjon):');
            disp(q_sol);

            if isempty(q_sol) || any(isnan(q_sol), 'all')
                warning('IK klarte ikke å finne en gyldig løsning – hopper over plotting.');
                mode = "done";   % eller hold på "pick" hvis en vil prøve igjen
                continue;        % hopp til neste iterasjon i while-løkka
            end

            % Hvis flere løsninger: ta første rad
            if size(q_sol,1) > 1
                q_sol = q_sol(1,:);
            end

            % For MATLAB-sim: visualiser armen (kan kommenteres bort i Gazebo-kjøring)
            figure(10); clf;
            robot.plot(q_sol, 'workspace', [-0.2 1.0 -0.5 0.8 0 1.2], ...
                       'view', [55 20], 'scale', 0.6);
            title('Arm posisjonert for plukk');
            
            % TODO: her, i Gazebo-versjonen, send q_sol til ROS2 joint controller

             % ---- PLANLEGG RETUR TIL SNAKE-BANEN ----
            % Finn punkt på original snake-bane nærmest posisjonen der vi avbrøt
         if ~isempty(boatPose_at_snake_interrupt)
            % vi bruker posisjonen der vi AVBRØT som mål på snake-banen
            pos_target = boatPose_at_snake_interrupt(1:2);
    
            % konverter både nåværende posisjon og target til grid-indekser
            res = 0.1;
            ix0  = round(boatPose(1) / res) + 1;
            iy0  = round(boatPose(2) / res) + 1;
            ith0 = 0;
            ...
            % Bytt "snake_path" midlertidig til retur-banen
            snake_path = W_back;
            s_path     = cumulativeArcLength(snake_path);
            s_prog     = 0;
        ...
        if ~isempty(boatPose_at_snake_interrupt)
            ...
            if dist_to_target < 0.2
                ...
                % Gjenopprett original snake-bane og fremdrift
                snake_path = snake_path_orig;
                s_path     = s_path_orig;
                s_prog     = s_prog_orig;
        ...
        case "done"
            % Her kan dere f.eks. stoppe båten:
            omega_cmd = 0;
            omega_act = 0;
            % eller bytte tilbake til snake-modus osv.
    end


    % --- IMU målinger (basert på sann omega_act og v) ---
    ...
    % --- EKF PREDIKSJON (bruk faktisk v og målt omega) ---
    % Jacobimatrise for posisjon og orientering
    F = [1 0 -v*dt*sin(x_hat(3));   % d x / d theta
         0 1  v*dt*cos(x_hat(3));   % d y / d theta
         0 0  1];

    % Tilstandsforutsigelse
    x_hat = x_hat + [v*cos(x_hat(3));             % Forutsier ny x-posisjon
                     v*sin(x_hat(3));             % Forutsier ny y-posisjon
                     (omega_meas - gyro_bias)] * dt;  % Oppdaterer heading basert på gyro 

    % Kovariansforutsigelse
    P = F*P*F' + Q;

    % --- EKF OPPDATERING 1 (magnetometer-heading) ---
    H = [0 0 1];                       % Målematrise: vi måler direkte på theta
    ...
    P = (eye(3)-K_mag*H)*P;            % Oppdaterer kovarians
...
e_ct = -sin(chi_path)*dx + cos(chi_path)*dy;                        % Beregner sideveis avstand til banen (tverravvik)
end
