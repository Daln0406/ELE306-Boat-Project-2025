function run_arm_hav_sweep()
% Kjører full hav-sveip og tømming av håv for 5DOF-armen på båt.
...
% BÅTGEOMETRI (kun for visualisering i denne figuren)
boat_length = 1.0;
boat_width  = 0.4;
boat_height = 0.2;
...
% Plassering av båten i vannet og armen på båten (kun grafikk)
...
% Løftet pose etter sveip (over sluttpunktet)
T_lift  = transl(x_center, y_right, z_level + lift_dz) * r2t(R_align);

% Invers kinematikk
mask = [1 1 1 0 0 1];   % x, y, z, Rz låst
...
% 2) Roter litt ekstra rundt Z for å få Y ≈ 0
T_start = robot.fkine(q_start);
p_start = transl(T_start);
x0 = p_start(1);
y0 = p_start(2);
z0 = p_start(3); %#ok<NASGU>
...
T_start = robot.fkine(q_start);
p_start = transl(T_start);
x_start = p_start(1); %#ok<NASGU>
y_start = p_start(2); %#ok<NASGU>
z_start = p_start(3); %#ok<NASGU>
...
% 0) Start -> løftepose over start
dq_pos = q_lift - q_right;
q_start_lift = q_start + dq_pos;
...
% 3) Etter sveip: løft og roter håven slik at X peker opp
q_end_sweep = q_traj2(end,:);
...
% 4) Løfter endeffektoren + holder orientert pose til sluttpose
...
% ================ VISUALISERING ==================
fig = figure('Color','w','Name','Håv-sveip');
robot.plot(q_traj(1,:), ...
...
plot3(x_center, y_left,  z_level,  'ro','MarkerSize',8,'LineWidth',2);
plot3(x_center, y_right, z_level,  'ko','MarkerSize',8,'LineWidth',2);
plot3(x_start,  y_start, z_start,  'bo','MarkerSize',8,'LineWidth',2);
