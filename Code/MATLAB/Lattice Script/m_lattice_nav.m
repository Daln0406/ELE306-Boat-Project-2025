...
lp = Lattice(occupancy, 'grid', 3, 'root', [1 1 0]);

% Konstruerer navigasjonsobjekt
...
lp.plot();  % Plotter lattice-griden uten path
ax0 = gca;
...
lp.plot(); % Plotter griden før path
ax1 = gca;
title(ax1,'Lattice planner (0.1 m/cell) — lattice');

disp('Original path: ')
lp.query(qstart, qgoal); % bruker eksakte noder fra grafen

lp.plot(); % Visualiser path
ax1 = gca;
title(ax1,'Lattice planner (0.1 m/cell) — original path');
...
% Høyere kostnader for svinger → foretrekker rette segmenter
lp.plan('cost', [1 10 10]);

disp('Increased cost path: ')
lp.query(qstart, qgoal);
